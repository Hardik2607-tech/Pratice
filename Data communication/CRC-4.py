...
def encode_data(data, key="10011"):  # Default CRC-4-ITU polynomial
    """Append CRC-4 checksum to data for transmission."""
    key_len = len(key)
    appended_data = data + '0' * (key_len - 1)  # Append zero bits
    remainder = mod2div(appended_data, key)
    return data + remainder  # Final transmitted message

def decode_data(received_data, key="10011"):  # Default CRC-4-ITU polynomial
    """Check for errors in received data using CRC-4 validation."""
    remainder = mod2div(received_data, key)
    return remainder == '0' * (len(key) - 1)  # If remainder is all zeros, data is valid

# Example Usage with CRC-4
data = input("Enter Data (binary): ")  # Example: "1101011011"
key = "10011"  # CRC-4-ITU Polynomial

print("\nOriginal Data:", data)
transmitted_data = encode_data(data, key)
print("Transmitted Data (with CRC-4):", transmitted_data)
